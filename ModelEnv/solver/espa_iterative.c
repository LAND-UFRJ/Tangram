...
/************************************************************************/
/* ESPA_itr: Efficient Transient State Probability Approximation	*/
...
/*									*/
/* Remark: Every vector, except for z and w, have its dimension stored	*/
...
/*	  for compatibility with the sor routine (module option_2.c).	*/
/************************************************************************/

double **ESPA_itr(Q, set_of_R, num_total_point,set_of_intervals,pi0,destv,uniformization_rate,solu_option,precision,max_no_iteration,stop_condition)
Matrix	*Q;	           /* ptr to generator matrix	                   */
...
int     *stop_condition;    /*stop_condition  0 - Max_Iterations 1- Conve  */ 
...
put_trans_matrix(P);
	...
	for (idest = 1; idest <= ndest; idest++)
	{
       ...
       /* initialize z with z(0) = 1i (ith element=1; all others=0)
       (it makes sense: prob{state(t) = dest | state (0) = j} when t -> 0).	*/
		for (k = 1; k <= N; k++)
			if (k == dest)
				z[k - 1] = 1;
			else	z[k - 1] = 0;
				    ...
				    for (k = 0; k < N; k++)
				         w[k] = b1 * z[k];
		                    ...
		                    switch (solu_option)
                                       {
                                            case (SOR_METHOD):
                                                               z = sor_flex(A, z, w, &iter, w_ini, w_max,precision,max_no_iteration,stop_condition);
				            
                                            break;
                   
                                            case (GAUSS_METHOD):
                                                               z = gauss_siedel(A, z, w, precision, &iter,max_no_iteration,stop_condition);
                                            
                                            break;
			   
			                    case (JACOBI_METHOD):
					                       z = jacobi(A, z, w, precision, &iter,max_no_iteration,stop_condition);
			   
			                    break;
                            
			                    case (POWER_METHOD):
					                       z = power(A, z, precision, &iter,max_no_iteration,stop_condition);
...
/************************************************************************/
/* ESPA_itr_set: Efficient Transient Set  Probability                   */
...
double *ESPA_itr_set(Q, set_of_R, num_total_point, set_of_intervals, pi0,rvec,uniformization_rate,solu_option,precision,max_no_iteration, stop_condition)
...
double  precision;       /*precision                                   */
double  max_no_iteration;/*max No of iterations- used in stop condition*/
int     *stop_condition; /*stop_condition 0 - Max_Iterations 1- Converg */  
