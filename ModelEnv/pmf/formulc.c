...
/*MODIFIED VERSION --- patched for boolean expressions, 2/27/99 */
/*A byte-compiler for mathematical functions */
/*Copyright (c) 1993-99 by Harald Helfgott        */
 ...
 /* but it MAY touch its end (i.e. *i_trans) without changing it.*/
{
 ...
 char *scan;
 UCHAR *tempu, *temp3;
 char *tempc; /* Created by Edmundo Grune and Luiz Hoffmann so tempu need not
                 to be used as (char *) in function strtod(), line 846 */
 char *temps;
 char tempch;
 double tempd;
 ...
 for(pars=0, scan=end-1; scan>=begin; scan--) {
  ...
  else if(!pars && (*scan == '<' || *scan == '>' || *scan == '='))
   break;
 }

 if(scan >= begin) {
   if(scan==begin || (*(scan-1) != '<' && *(scan-1) != '>'))
     TWO_OP
       else {
    if((tempu=i_trans(function,begin,scan-1)) &&      
       (temp3=i_trans(tempu,scan+1,end)) ) {       
         *temp3++ = (((*scan) == '>') ? 'n' :
		     (((*(scan-1)) == '<') ? 'l' : 'g'));
