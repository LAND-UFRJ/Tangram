...
/**
*	@file chained_list.cpp
*   @brief <b> Chained list data structure </b>.
*   @remarks Lot of.
*   @author LAND/UFRJ
*   @date 1999-2009
...
Chained_Element *Chained_Element::get_next()
{
  return( show_next() );
}
/***************************************************************************/
...
{
  if (access == LIST_RO) {
    debug (3, "Chained_List: Trying to add an element to a RO list.");
    return (-1);
  }
...
Chained_Element *Chained_List::del_head()
{
  ...
  if (access == LIST_RO) {
    debug (3, "Chained_List: Trying to delete an element from a RO list.");
    return (NULL);
  }
...
{
  if (access == LIST_RO) {
    debug (3, "Chained_List: Trying to replace an element in a RO list.");
    return (-1);
  }
