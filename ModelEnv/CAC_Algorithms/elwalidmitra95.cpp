...
int usage(char *name)
{
  ...
  printf ("Usage: ");
  printf ("%s -b <buffer> -c <capacity> -l <lost> > <peak_rate> <mean_rate> <MBS> \n", name);
  ...
  printf ("\t<mean_rate>\t Mean rate of the connection\n");    
  printf ("\t<MBS>\t Maximum Burst Size\n");
  return 1;
}
...
int parse_args(char **argv) 
{
  ...
  peak   = (double) atof(argv[i]);     /* Peak */
  i++;
  
  mean = (double) atof(argv[i]);     /* Mean */  
  i++;
  
  mbs = (double) atof(argv[i]);     /* MBS     */
...
int main(int argc, char* argv[])
{
  ...
  admission->regul_calc_prob_overflow ( peak, mean, mbs );
