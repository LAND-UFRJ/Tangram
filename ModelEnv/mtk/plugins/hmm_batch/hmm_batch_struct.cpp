...
bool HMMBatch::set_full( Arguments & args )
{
...
bool HMMBatch::set_coxian( Arguments & args )
{
    destroy_vector( parameter.N, parameter.pi );       
    destroy_matrix( parameter.N, parameter.N, parameter.A );
    destroy_matrix( parameter.N, parameter.M, parameter.r );
...
bool HMMBatch::set_qbd( Arguments & args )
{
...
void HMMBatch::fix_obs( void )
{
    ...
    for( i = 0; i < parameter.N; i++ )
    {
        ...
        {
            parameter.r[i][j] += epsilon;

            for( k = 0; k < parameter.M; k++ )
            {
                parameter.p[i][j][k] += epsilon;
            }
...
void HMMBatch::fix_qbd_matrix( int G, int E,
                           int B, int F )
{
    int n,
...
bool HMMBatch::fix_full( Arguments & args )
{
    unsigned int i, j;
    ...
    {
        Terminal::displayErrorMessage("parameter values differ from model's values.");
        return false;
    }
...
bool HMMBatch::fix_coxian( Arguments & args )
{
    unsigned int i, j;
...
bool HMMBatch::fix_qbd( Arguments & args )
{
    int n_groups, n_elem, back, fwd;  
    unsigned int i, j;
...
bool HMMBatch::set_epsilon( Arguments & args )
{
