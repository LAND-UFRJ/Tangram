...
{
    "hmm_batch",
    "Hierarquical General Hidden Markov Model - Fixed Batch",
        ...
        : ObjectBase<HMMBatch>(name,this)
{
    ...
    Terminal::displayMessage( "hmm_batch object was successfully created with name %s", getName() );
...
HMMBatch::HMMBatch( const Name & name, unsigned int n, unsigned int m, unsigned int b )
        : ObjectBase<HMMBatch>(name,this)
{
    parameter.N = n;
    parameter.M = m;
...
bool HMMBatch::display_pi( Terminal & term )
{
    ...
    term.print( "Hidden initial state distribution:\n" );
    term.print( " [" );
...
bool HMMBatch::exec_simulate( Arguments & args )
{
    ...
    for( base = 0, b = 0; b < sample_states.size; b++, base += parameter.B )
    {
        ...
        sample_obs.data[base] = get_random( parameter.M, parameter.r[sample_states.data[b]] );

        for( i = 1; i < B; i++ )
        {
            // Sampling next observation according to p
            // Previous obs: sample_obs.data[base+i-1]
            // Actual obs: sample_obs.data[base+i]
            // Hidden State: sample_states.data[b]
            
            sample_obs.data[base+i] = 
                get_random( parameter.M, parameter.p[sample_states.data[b]][sample_obs.data[base+i-1]] );
        }

        // If has another batch, do transition between hidden states
        if( b < sample_states.size - 1 )
        {
...
Object * HMMBatch::createSized( const Name & name, Arguments & args )
{
    ...
    LOWER_LIMIT(M,1);
    B = atol( args[2].text.c_str( ) );
    LOWER_LIMIT(B,1);

    return new HMMBatch( name, N, M, B );
}
...
void initHMMBatch( void )
{
    ...
    HMMBatch::addConstructor( &HMMBatch::createSized,
                          Prototype( 3, 3, TK_INTEGER, TK_INTEGER, TK_INTEGER ) );

    HMMBatch::addOption( "N", "number of hidden states",
                     0, &HMMBatch::set_N,  &HMMBatch::get_N );
    HMMBatch::addOption( "M", "number of observations",
                     0, &HMMBatch::set_M,  &HMMBatch::get_M );
    HMMBatch::addOption( "B", "observation batch size",
                     0, &HMMBatch::set_B,  &HMMBatch::get_B );
