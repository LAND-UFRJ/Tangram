...
bool GHMM::load_all( const std::string &file_name )
{
    unsigned int i, j;
    Parameter new_param;
    ...
    if( infile )
    {
        ...
        READ(infile,new_param.B);
        LOWER_LIMIT(new_param.B,1);
        ...
        /* Loading Symbol Parameters */
        new_param.r = create_zero_vector( new_param.N );
        new_param.p = create_zero_vector( new_param.N );
        new_param.q = create_zero_vector( new_param.N );
        param_watcher.addWatch( new_param.r );
        param_watcher.addWatch( new_param.p );
        param_watcher.addWatch( new_param.q );

        for( i = 0; i < new_param.N; i++ )
        {
            READ(infile,new_param.r[i]);
            LOWER_LIMIT(new_param.r[i],0.0);
            UPPER_LIMIT(new_param.r[i],1.0);

            READ(infile,new_param.p[i]);
            LOWER_LIMIT(new_param.p[i],0.0);
            UPPER_LIMIT(new_param.p[i],1.0);

            READ(infile,new_param.q[i]);
            LOWER_LIMIT(new_param.q[i],0.0);
            UPPER_LIMIT(new_param.q[i],1.0);
        }
...
bool GHMM::load_pi( const std::string &file_name )
{
    unsigned int i;
    Parameter new_param;
    WatchDog<double> param_watcher;
...
bool GHMM::load_A( const std::string &file_name )
{
    unsigned int i, j;
    Parameter new_param;
    WatchDog<double> param_watcher;
...
bool GHMM::load_obs( const std::string &file_name )
{
    unsigned int i;
    Parameter new_param;
    WatchDog<double> param_watcher;
...
bool GHMM::load( Arguments & args )
{
    ...
    Terminal::displayMessage( "Ghmm was succefully loaded." );

    return true;
}

bool GHMM::save_all( const std::string &file_name )
{
    unsigned int i, j;
    ...
    if( outfile )
    {
        ...
        for( i = 0; i < parameter.N; i++ )
        {
            WRITE(outfile,parameter.r[i]);
            outfile << ' ';
            WRITE(outfile,parameter.p[i]);
            outfile << ' ';
            WRITE(outfile,parameter.q[i]);
            outfile << '\n';
        }
...
bool GHMM::save_pi( const std::string &file_name )
{
    unsigned int i;
    std::ofstream outfile( file_name.c_str( ) );
...
bool GHMM::save_A( const std::string &file_name )
{
    unsigned int i, j;
    std::ofstream outfile( file_name.c_str( ) );
...
bool GHMM::save_obs( const std::string &file_name )
{
    unsigned int i;
    std::ofstream outfile( file_name.c_str( ) );
...
bool GHMM::save( Arguments & args )
{
    ...
    Terminal::displayMessage( "Ghmm was succefully saved in file %s.", filename.c_str( ) );
 ...
 */
bool GHMM::import_from_tangram( Arguments &args )
{
    register int   i, pos;
    ...
    if( terminal )
    {
        ...
        /* Check if all internal models have two states */
        for( i = 0; i < hidden_model->num_hidden_states; i++ )
        {
            if( hidden_model->num_internal_states[i] != 2 )
            {
                Terminal::displayErrorMessage( "The model inside hidden state"
                                               " %d does not have two states!"
                                               " Aborting...", i + 1 );
                return false;                
            }
        }

        /* Reset parameters. PI will be given random values, for the user will,
           later, load it. */
        destroy_vector( parameter.N, parameter.pi );
        destroy_matrix( parameter.N, parameter.N, parameter.A );
        destroy_vector( parameter.N, parameter.r  );
        ...
        parameter.r  = create_zero_vector( parameter.N );        
        parameter.p  = create_zero_vector( parameter.N );        
        parameter.q  = create_zero_vector( parameter.N );
 ...
 */
bool GHMM::import_sparse_A( char *file_name )
{
    int               state_a, state_b;    
    double            prob;  
    WatchDog<double>  param_watcher;
 ...
 */
bool GHMM::import_sparse_initial_prob( int   hidden_state,
                                       char *file_name )
{
    int               internal_state;    
    double            prob;  
    ...
    if( fd )
    {
        param_watcher.addWatch( parameter.r );
        ...
        while( fscanf( fd, "%d %lf\n", &internal_state, &prob ) == 2 )
        {
            ...
            // Get probability of symbol 1
            if( internal_state == 1 )
            {
                parameter.r[ hidden_state ] = prob;
                LOWER_LIMIT( parameter.r[ hidden_state ], 0.0 );
                UPPER_LIMIT( parameter.r[ hidden_state ], 1.0 );
            }
        }        
 ...
 * from 'file_name'
 */
bool GHMM::import_sparse_transitions( int   hidden_state,
                                      char *file_name )
{
    ...
    if( fd )
    {
        param_watcher.addWatch( parameter.p );
        param_watcher.addWatch( parameter.q );
                       ...
                       &prob ) == 3 )
        {
            ...
            // Get a transition from 0 to 1
            if( internal_state_a == 0 &&
                internal_state_b == 1 )
            {
                parameter.p[ hidden_state ] = prob;
                LOWER_LIMIT( parameter.p[ hidden_state ], 0.0 );
                UPPER_LIMIT( parameter.p[ hidden_state ], 1.0 );
            }
            // Get transition from 1 to 0
            else if( internal_state_a == 1 &&
                     internal_state_b == 0 )
            {
                parameter.q[ hidden_state ] = prob;
                LOWER_LIMIT( parameter.q[ hidden_state ], 0.0 );
                UPPER_LIMIT( parameter.q[ hidden_state ], 1.0 );
            }
        }        
