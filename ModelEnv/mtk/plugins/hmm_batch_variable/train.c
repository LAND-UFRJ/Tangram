...
#define BATCH(t) (batches[t].index)
...
#define Eval_Csi( param, obs, t, i, j ) \
     (ALPHA(IDX(t,batches[t].size-1),i) * param->A[i][j] * \
...
#define Eval_Gamma( param, t, i ) \
     (ALPHA(IDX(t,batches[t].size-1),i) * BETA(IDX(t,batches[t].size-1),i) / \
      SCALE(IDX(t,batches[t].size-1)))
